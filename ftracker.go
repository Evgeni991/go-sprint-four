...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	speed := kmhInMsec * meanSpeed(action, duration)
	speedSquared := math.Pow(speed, 2)
	return ((walkingCaloriesWeightMultiplier*weight + speedSquared/(height/cmInM)*walkingSpeedHeightMultiplier*weight) * duration * minInH)
}
